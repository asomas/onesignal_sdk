...
class TestAsyncBasicAuthRequest:
    ...
    async def test_raises_onesignalhttperror_for_error_status_codes(self):
        with mock.patch(
            ...
            side_effect=mock_request(is_async=True, response=self.RESPONSE_404),
        ):
    ...
    async def test_returns_response_and_pass_body_for_valid_request(self):
        with mock.patch(
            ...
            response = await async_basic_auth_request(
                "GET", self.TEST_URL, self.TOKEN, {"name": "test"}, {"offset": 3}
            )
